...
export default function QuickAddSupplier({ existingSuppliers, onCreated, onClose }) {
  const [form, setForm] = useState({ name: "", contactPerson: "", phone: "", mobile: "", email: "", address: "", state: "", pin: "", gstNumber: "", notes: "" });
  ...
  useEffect(() => {
    ...
    if (gstin && gstin.length >= 15) {
      match = existingSuppliers.find(s => normalize(s.gstNumber) === gstin);
    }
    if (!match && phone && phone.length >= 6) {
      match = existingSuppliers.find(s => normalize(s.phone) === phone || normalize(s.mobile) === phone);
    }
    if (!match && name && name.length >= 3) {
      match = existingSuppliers.find(s => normalize(s.name) === name);
    }
    setDuplicateWarning(match ? `Supplier may already exist: "${match.name}"${match.gstNumber ? ` (GSTIN: ${match.gstNumber})` : ""}` : null);
  }, [form.name, form.phone, form.mobile, form.gstNumber, existingSuppliers]);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (form.contactPerson) payload.contactPerson = form.contactPerson.trim();
      ...
      if (!res.ok) throw new Error(data.error?.message || "Failed to create supplier");
      onCreated(data);
      ...
      <div className="modal-content quick-add-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            <div className="form-group"><label>Supplier Name *</label><input ref={nameRef} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. ABC Suppliers" /></div>
          </div>
          <div className="form-row">
            <div className="form-group"><label>Contact Person</label><input value={form.contactPerson} onChange={(e) => setForm({ ...form, contactPerson: e.target.value })} placeholder="Contact person name" /></div>
          </div>
          <div className="form-row">
          ...
          <div className="form-row">
            ...
            <div className="form-group"><label>GSTIN</label><input value={form.gstNumber} onChange={(e) => setForm({ ...form, gstNumber: e.target.value })} placeholder="15-digit GSTIN" maxLength={15} /></div>
          </div>
          ...
          <div className="form-row">
            <div className="form-group"><label>State</label><input value={form.state} onChange={(e) => setForm({ ...form, state: e.target.value })} placeholder="e.g. Karnataka" /></div>
          ...
          <div className="quick-add-actions">
            <button className="btn btn-primary" onClick={save} disabled={saving || !form.name.trim()}>{saving ? "Saving…" : "Save Supplier"}</button>
